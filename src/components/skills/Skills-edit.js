import React,{useState,useEffect} from "react";
import {useLocation,useNavigate} from "react-router-dom";
import axios from "axios";

const SkillsEdit=(props)=>{
   const navigate=useNavigate();
   const {state}=useLocation();
   console.log("ssss.",state)
   const [payload,setPayload]=useState([]);

   useEffect(()=>{
      axios.post("http://localhost:4000/get-skillById",state).then((response)=>{
            setPayload(response?.data?.skillData)


      });
},[]);

const handleInputChange=(e)=>{
   setPayload({
      ...payload,
      [e.target.id]:e.target.value
   });

};

const handleSubmit=(e)=>{
   
   axios.put("http://localhost:4000/update-skill",payload).then((response)=>{
      if (response?.data) {
         console.log("ppp..",payload)

         console.log("record updated");
         navigate("/view-skills");
         setPayload({});
       }


   })


}

 return (
   <div className="row">
   <div className="col-md-6">
     <form>
       <fieldset>
         <legend>My Skills</legend>

         <div className="form-group">
           <label htmlFor="exampleInputPassword1">Title</label>
           <input
             type="text"
             defaultValue={payload?.title}
             className="form-control"
             id="title"
             placeholder="Title"
             onChange={handleInputChange}
           />
         </div>

         <div className="form-group">
           <label htmlFor="exampleInputPassword1">Total Percentage</label>
           <input
             type="float"
             defaultValue={payload?.totalPercentage}
             className="form-control"
             id="totalPercentage"
             placeholder="Total Percentage"
             onChange={handleInputChange}
           />
         </div>

         <div className="form-group">
           <label htmlFor="exampleInputPassword1">
             Last Week Percentage
           </label>
           <input
             type="float"
             defaultValue={payload?.lastWeekPercentage}

             className="form-control"
             id="lastWeekPercentage"
             placeholder="Last Week Percentage"
             onChange={handleInputChange}
           />
         </div>

         <div className="form-group">
           <label htmlFor="exampleInputPassword1">
             Last Month Percentage
           </label>
           <input
             type="float"
             defaultValue={payload?.lastMonthPercentage}

             className="form-control"
             id="lastMonthPercentage"
             placeholder="Last Month Percentage"
             onChange={handleInputChange}
           />
         </div>

         <button
           type="button"
           className="btn btn-primary"
           onClick={(e)=>{handleSubmit(e)}}
         >
           Submit
         </button>
         
       </fieldset>
     </form>
   </div>
   <div className="col-md-6"></div>
 </div>

 );



};

export default SkillsEdit;